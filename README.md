...
Five hands-on build-alongs. Each table builds **one** real AI agent - in Python (standard library only), on OpenAI or Anthropic models -
...
- That's it. No packages to install - the notebooks call the model over HTTPS with the Python standard library, so they work on locked-down machines.
- Pick the model in `.env` with `WORKSHOP_MODEL` (any OpenAI or Anthropic model id). Recommended: `gpt-4o` (OpenAI) or `anthropic/claude-sonnet-5` (Anthropic).
